...
def draw_Arrow_Out(list_Type, ToRefineAnd, ToRefineOr, index, net, list_Vector, list_Transition, list_Goal):
    if list_Type == 'ToRefineAnd':
        cadena = ToRefineAnd[index]
        separador = ';'
        separador = cadena.split(separador)
        indice_1 = list_Vector.index(str(separador[0]))
        indice_2 = ToRefineAnd.index(cadena)

        # Arc 1
        # Son 1.4
        arc = ET.SubElement(net, 'arc') 
        arc.set('id', 'T' + str(list_Transition[indice_1]) + ' to ' + list_Goal[indice_2].place_Id())
        arc.set('source', 'T' + str(list_Transition[indice_1]))
        arc.set('target', list_Goal[indice_2].place_Id())
