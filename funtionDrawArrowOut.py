...
def draw_Arrow_Out(list_Transition, index, net, output_Vector_List):
    if list_Transition[index - 1 ] != list_Transition[index]:
        # Arc 1
        # Son 1.4
        arc = ET.SubElement(net, 'arc') 
        arc.set('id', 'T' + str(list_Transition[index]) + ' to ' + output_Vector_List[index])
        ''' print('position teste and out')
        print('T' + str(list_Transition[indice_1]) + ' to ' + list_Goal[indice_2].place_Id()) '''
        arc.set('source', 'T' + str(list_Transition[index]))
        arc.set('target', output_Vector_List[index])
